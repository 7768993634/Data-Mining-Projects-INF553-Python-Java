...
if __name__ == "__main__":
    ...
    temp2 = temp.flatMap(lambda x: [(str(i) + '_' + str(x[1].index(i)), x[0]) for i in x[1]]).groupByKey().map(lambda x: (x[0], list(x[1]))).filter(lambda x: len(x[1]) > 1).mapPartitions(func).reduceByKey(lambda x,y: list(set(x) | set(y)))
